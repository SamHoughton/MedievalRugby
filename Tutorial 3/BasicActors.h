...
{
	///Plane class which is also the field in this game
	...
	///Sphere class will be used for the ball in the game 
	...
	///bottom class for one of the edges of the arena
	...
	/// Both side walls of the arena 
	class Wall : public DynamicActor
	{
	public:


		// Adding and creating the battlements onto the wall
	...
	/// The back wall of the arena
	class backWall : public DynamicActor
	{
	public:

		// The back wall with the battlements of them
	...
	// The contact box for the scoring in the game
	class HiddenBox : public DynamicActor
	{
	public:

		// Set the boxes size to be behind the goal
		HiddenBox(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(5.5f, 9.5f, 1.5f), PxReal density = 500.f)
			: DynamicActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions), density);
		}
	...
	// The top of the arena for the game
	...
	// The flags in the game (these had clothes but had to be removed due to lag from the flags)
	class Flag : public DynamicActor
	{
	public:
		Flag(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, 3.5f, .5f), PxReal density = 50.f)
			: DynamicActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions), density);
		}
	...
	// Horse clas to create the horse obstacles
	...
	// Main movement catapult for the player
