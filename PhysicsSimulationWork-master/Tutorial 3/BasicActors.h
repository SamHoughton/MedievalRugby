...
namespace PhysicsEngine
{
	...
	{
	public:
		Horse(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(1.f, .5f, 1.5f), PxReal density = 5.f)
			: DynamicActor(pose)
		{
			// Main base of horse
			CreateShape(PxBoxGeometry(dimensions), density);
			// Legs of horse
			CreateShape(PxBoxGeometry(.2f, 0.9f, .2f), density);
			CreateShape(PxBoxGeometry(.2f, 0.9f, .2f), density);
			CreateShape(PxBoxGeometry(.2f, 0.9f, .2f), density);
			CreateShape(PxBoxGeometry(.2f, 0.9f, .2f), density);
			// Tail of horse
	...
	public:
			...
			: DynamicActor(pose)
		{
			...
			CreateShape(PxBoxGeometry(.5f, 6.f, 0.5f), 10.1f);
			CreateShape(PxBoxGeometry(.5f, 6.f, 0.5f), 10.1f);
